...
public class TownGUI extends JFrame implements ActionListener{
	...
	private Vector<Pair<String, Integer>> companiesInTown;
	...
	private static final int[] portsOfArrivingHelicopters = {55000,56000,57000};
	...
	public TownGUI(int port,Vector<Pair<String,Integer>> companies){
		this.setupGUI();
		this.myPort = port;
		this.companiesInTown = companies;
	...
	public void actionPerformed(ActionEvent event) {
		...
		Socket socket;
		Pair<Packet,Integer> pairToSendToChopper; 
		 ...
		 if (event.getSource()==this.bookButton){
				...
				if(this.areDatesValid()){
					...
					if(!namePair.isNull()){
						...
						Packet packToSend = new Packet(this.firstname.getText(),this.surname.getText(),date,null,(String)this.directionComboBox.getSelectedItem());
								
						Pair<Packet,Boolean> response = this.sendPacketToTown(packToSend);
						Packet rPacket = response.getFirstElement();	
						boolean rBoolean = response.getSecondElement();	
						System.out.println(rPacket.toString());
						if(rBoolean){			
							System.out.println("No Errors Occured");
						
						}else{
							System.out.println("Networking Errors Occured. Try again");
						}
					...
					Packet packToSend = new Packet(cancelationReferenceNumber);
					Pair<Packet,Boolean> responsePair=this.sendPacketToTown(packToSend);
					Packet responsePacket    = responsePair.getFirstElement(); 
					boolean responseB = responsePair.getSecondElement();
					System.out.println(responsePacket.toString());
					if(responseB){
						System.out.println("Success");
					}else{
						System.out.println("Network errors occured");
					}
		}
		}
	...
	private Pair<Packet,Boolean>sendPacketToTown(Packet packetToSend){
		...
		for(Pair<String,Integer> ipStringPair :this.companiesInTown){
			   ...
			   try {
				...
				if(packetToSend.equals("CancelationPacket")){
					if(response.getPacketType().equals("CancelationConfirmationPacket")){
						//System.out.println("Returning Cancelation Confirmation");
						return new Pair<Packet,Boolean>(response,true);
					}else{
						continue;
					}
	...
	public static void main(String args[]){
		...
		if(args.length!=0){
			@SuppressWarnings("unused")
			TownGUI tgui = new TownGUI(Integer.parseInt(args[0]),vec);
		}else{
			@SuppressWarnings("unused")
			TownGUI tgui = new TownGUI(Integer.parseInt("60000"),vec);
		}
	}
